    ...
    <div className="p-4 md:p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <h1 className="text-xl md:text-2xl font-bold bg-gradient-to-r from-white to-slate-300 bg-clip-text text-transparent line-clamp-2">{course?.title} - Quizzes</h1>
        </div>
      ...
      <div className="bg-slate-800/50 backdrop-blur rounded-xl border border-slate-700/50">
        {quizzes.length === 0 ? (
          ...
          <ul className="divide-y divide-slate-700/50">
            {quizzes.map((quiz) => (
              <li key={quiz.id} className="p-3 md:p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-slate-700/30 transition-colors">
                <div className="min-w-0">
                  <div className="font-medium text-white truncate">{quiz.title}</div>
                  <div className="text-sm text-slate-400 flex flex-wrap items-center gap-x-2">
                    <span>{quiz._count?.questions || 0} questions</span>
                    {quiz.timeLimitSeconds && <span>• {Math.floor(quiz.timeLimitSeconds / 60)} min limit</span>}
                    {quiz.attemptsAllowed && <span>• {quiz.attemptsAllowed} attempts</span>}
                    {quiz.isRequired && <span className="text-orange-400">Required</span>}
                  </div>
                </div>
                <div className="flex gap-2 ml-auto sm:ml-0">
                  <Link to={`/lms/courses/${courseId}/quizzes/${quiz.id}/questions`} className="text-purple-400 hover:text-purple-300 px-2 md:px-3 py-1 transition-colors text-sm">Questions</Link>
                  <button onClick={() => openModal(quiz)} className="text-blue-400 hover:text-blue-300 px-2 md:px-3 py-1 transition-colors text-sm">Edit</button>
                  <button onClick={() => handleDelete(quiz.id)} className="text-red-400 hover:text-red-300 px-2 md:px-3 py-1 transition-colors text-sm">Delete</button>
                </div>
              </li>
            ))}
          </ul>
      ...
      {showModal && editingQuiz && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-2xl border border-slate-700/50 p-4 md:p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            <h2 className="text-lg md:text-xl font-bold mb-4 text-white">{editingQuiz.id ? 'Edit Quiz' : 'Add Quiz'}</h2>
