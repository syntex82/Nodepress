          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              {cart.items.map((item) => {
                ...
                const itemLink = isCourse
                  ? `/lms/course/${item.course?.slug}`
                ...
                <div key={item.id} className="bg-white rounded-2xl shadow-lg shadow-slate-200/50 p-6 hover:shadow-xl transition-shadow">
                  <div className="flex gap-6">
                    <div className="w-28 h-28 bg-gradient-to-br from-slate-100 to-slate-50 rounded-xl overflow-hidden flex-shrink-0 relative">
                      {item.image ? (
                      ...
                      {isCourse && (
                        <div className="absolute top-2 left-2 bg-indigo-600 text-white text-xs px-2 py-0.5 rounded-full">Course</div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <Link to={itemLink} className="text-lg font-semibold text-slate-900 hover:text-violet-600 transition-colors block truncate">
                        {item.name}
                      </Link>
                      {item.variant && <p className="text-sm text-slate-500 mt-1">{item.variant.name}</p>}
                      {isCourse && item.course?.instructor?.name && (
                        <p className="text-sm text-slate-500 mt-1">by {item.course.instructor.name}</p>
                      )}
                    ...
                    <div className="flex flex-col items-end justify-between">
                      ...
                      {isCourse && (
                        <div className="text-sm text-slate-500">Lifetime access</div>
                      )}
                    </div>
                  </div>
                </div>
